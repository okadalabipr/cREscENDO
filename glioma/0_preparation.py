...
np.save(samplename+"/atac_count_hg19.npy",m)

peakbed_s=peakbed[(atac.var['chr'].isin(chrs)),:]
np.savetxt(samplename+"/peaks_hg19.bed", peakbed_s, delimiter='\t', fmt='%s')

peaklist=pd.read_csv(samplename+"/peaks_hg19.bed",sep="\t",header=None,names=("chr","start","end"))
peaklist["tag"]=peaklist["chr"]+str("-")+peaklist["start"].astype('str')+str("-")+peaklist["end"].astype('str')
peaklist.to_csv(samplename+"/peaks_hg19_tag.bed", sep="\t",header=False, index=False)