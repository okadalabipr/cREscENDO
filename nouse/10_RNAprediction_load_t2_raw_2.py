...
pairlist_train=pd.read_csv(samplename+"/pair_300000_train.csv",header=None,names=("gene","start","end","pos"))
pairlist_test=pd.read_csv(samplename+"/pair_300000_test.csv",header=None,names=("gene","start","end","pos"))
...
embedlist=pd.read_csv(samplename+"/embed.txt",sep=" ",header=None)
...
transformernet.load_state_dict(torch.load(samplename+"/transformer_rnapred_transformer_t2_raw_2.pth"))
rnanet.load_state_dict(torch.load(samplename+"/transformer_rnapred_rna_t2_raw_2.pth"))
...
for data, prom, atac, rna, genet in full_batch:
    ...
    atacvec_g=atac.detach().requires_grad_(True)

    # データを入力して予測値を計算（順伝播）
    output=transformernet(data,prom,atacvec_g)
    ...
    rna=rna.flatten()
    pred_i=pred_o.flatten()
    pred_i_lis.append(pred_i.view(-1).tolist())
    true_x_lis.append(rna.view(-1).tolist())

    pred_out=pred_o.detach().clone()
    loss=rnalosscal_grad(pred_o,pred_out)
    loss.backward()

    enhagrad=torch.mul(atac,relu(atacvec_g.grad))
    grad_lis.append(enhagrad)
    #prom_list.append(atac[:,:,0])
...
f = open(samplename+"/true_expression_t2_raw.txt", "w")
for x in true_x_lis:
    f.write(str(x) + "\n")
f.close()


gradm_m=torch.cat(grad_lis, dim=0) #gene,cell,len
outn = gradm_m.to("cpu").detach().numpy().copy()
np.save(samplename+"/gradcam_score_t2_raw_2.npy",outn)

#prom_m=torch.cat(prom_list, dim=0) #gene,cell,len
#outn = prom_m.to("cpu").detach().numpy().copy()
#np.save("/sqfs/work/K2313/u6b872/umap_pred/promatac_promsep_log_enhacom_norm5.npy",outn)

#atac_m=torch.cat(atac_list, dim=0) #gene,cell,len
#outn = atac_m.to("cpu").detach().numpy().copy()
#np.save("/sqfs/work/K2313/u6b872/umap_pred/atac_promsep_log_enhacom_norm5.npy",outn)

#rna_m=torch.cat(rna_list, dim=0) #gene,cell,len
#outn = rna_m.to("cpu").detach().numpy().copy()
#np.save(samplename+"/predrna.npy",outn)