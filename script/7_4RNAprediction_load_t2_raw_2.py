...
with torch.no_grad():
  for data, prom, atac, rna, genet in full_batch:
      ...
      pred_i_lis.append(pred_o)
      true_x_lis.append(rna)

pred_mtx=torch.cat(pred_i_lis, dim=0) #gene,cell,len
outn = pred_mtx.to("cpu").detach().numpy().copy()
np.save(samplename+"/pred_mtx_fold"+str(chunkid)+".npy",outn)

true_mtx=torch.cat(true_x_lis, dim=0) #gene,cell,len
outn = true_mtx.to("cpu").detach().numpy().copy()
np.save(samplename+"/true_mtx_fold"+str(chunkid)+".npy",outn)